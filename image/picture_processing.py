...
TIFF_IMAGE_FOLDERS = {
    "900W": os.path.join(DATA_DIR, "OM", "laser900W"),
    "1200W": os.path.join(DATA_DIR, "OM", "laser1200W"),
    "1500W": os.path.join(DATA_DIR, "OM", "laser1500W"),
    "1800W": os.path.join(DATA_DIR, "OM", "laser1800W")
}
...
class RegressionModels:
    ...
    def __init__(self, target_col=None, progress_callback=None):
        self.target_col = target_col or "mh_mean_hv"
        self.models = {}
        self.scalers = {}
        self.best_params = {}
        self.feature_names = []
...
def run_ml_pipeline(csv_path=None, quant_df=None, progress_callback=None):
    ...
    if quant_df is None:
        if csv_path is None:
            full_csv_path = os.path.join(SAVE_RESULT_FOLDER, "完整实验数据汇总.csv")
            basic_csv_path = os.path.join(SAVE_RESULT_FOLDER, "金相定量表征数据汇总.csv")
            if os.path.exists(full_csv_path):
                csv_path = full_csv_path
                print("  优先加载完整实验数据汇总.csv（包含实测硬度数据）")
            elif os.path.exists(basic_csv_path):
                csv_path = basic_csv_path
                print("  加载金相定量表征数据汇总.csv")
            else:
                print(f"错误: CSV文件不存在 - {full_csv_path} 或 {basic_csv_path}")
                return None
        if not os.path.exists(csv_path):
            print(f"错误: CSV文件不存在 - {csv_path}")
            return None
        quant_df = pd.read_csv(csv_path, encoding="utf-8-sig")
    ...
    reg_models = RegressionModels(target_col="mh_mean_hv", progress_callback=progress_callback)
    if progress_callback:
    ...
    # 绘制Pearson相关系数矩阵
    print("\n绘制Pearson相关系数矩阵...")
    cols = reg_models.feature_names + [reg_models.target_col]
