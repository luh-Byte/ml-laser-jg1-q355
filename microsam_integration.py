...
def create_streamlit_ui():
    """
    为 Streamlit 创建 micro_sam UI 组件
    注意: 此函数应在 Streamlit 应用中调用
    """
    try:
        import streamlit as st

        st.subheader("🔬 micro_sam 显微图像预训练模型")

        # 检查安装状态
        installed = check_micro_sam_installed()

        if not installed:
            st.warning("⚠️ micro_sam 未安装")
            with st.expander("查看安装说明"):
                st.code(install_micro_sam_instructions(), language="python")
            return None

        st.success("✅ micro_sam 已安装")

        # 模型选择
        model_type = st.selectbox(
            "选择模型类型",
            options=list(MICRO_SAM_MODELS.keys()),
            format_func=lambda x: MICRO_SAM_MODELS[x],
            index=2  # 默认 vit_b
        )
        ...
        # 分割模式
        auto_mode = st.checkbox("自动分割模式", value=True)

        # 创建分割器
        if st.button("加载模型"):
            segmenter = MicroSAMSegmenter(model_type=model_type, device=device)
            st.session_state.micro_sam_segmenter = segmenter
            st.success(f"模型 {model_type} 已加载")

        return st.session_state.get("micro_sam_segmenter")

    except ImportError:
        print("Streamlit 未安装，无法创建 UI")
        return None
